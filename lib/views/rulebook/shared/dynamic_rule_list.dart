...
class DynamicRuleList extends StatefulWidget {
  final String title;
  final String jsonPathEn;
  final String? jsonPathTr;

  const DynamicRuleList({
    super.key,
    required this.title,
    required this.jsonPathEn,
    this.jsonPathTr,
  });
...
class _DynamicRuleListState extends State<DynamicRuleList> {
  ...
  String? _lastLocale;
  String _overviewLabel = 'Overview';
  Map<String, String> _headerTitleMap = {};

  String _localizedTitle(BuildContext context) {
    return switch (widget.title) {
      'Races' => context.tr('rulebook.races'),
      'Classes' => context.tr('rulebook.classes'),
      'Equipment' => context.tr('rulebook.equipment'),
      'Monsters' => context.tr('rulebook.monsters'),
      'Spells' => context.tr('rulebook.spells'),
      _ => widget.title,
    };
  }

  String _featuredDescription(BuildContext context) {
    return switch (widget.title) {
      'Spells' => context.tr('featuredSpells'),
      'Equipment' => context.tr('featuredEquipment'),
      'Monsters' => context.tr('featuredMonsters'),
      _ => context.tr('featuredDefault'),
    };
  }

  String _resolveJsonPath(BuildContext context) {
    final locale = Localizations.localeOf(context).languageCode;
    if (locale == 'tr' && widget.jsonPathTr != null) {
      return widget.jsonPathTr!;
    }
    return widget.jsonPathEn;
  }

  // Türkçe ırk/sınıf isimlerini İngilizce dosya adlarına eşle
  static const _trToEnImageMap = {
    // Races
    'cüce': 'dwarf',
    'elf': 'elf',
    'buçukluk': 'halfling',
    ...
    'korucu': 'ranger',
    'düzenbaz': 'rogue',
    'büyüdoğumlu': 'sorcerer',
    'pakt büyücüsü': 'warlock',
    'büyücü': 'wizard',
  };
  ...
  String? _getRuleImageUrl(String name) {
    ...
    if (lowerName.contains('traits') ||
        ...
        lowerName.contains('özellikleri') ||
        lowerName.contains('kuralları'))
      return null;

    final categoryTitle = widget.title.toLowerCase().trim();

    // Önce Türkçe→İngilizce eşleme tablosuna bak, yoksa doğrudan ismi kullan
    final mappedName = _trToEnImageMap[lowerName.trim()];
    final itemName = mappedName ?? name
        .trim()
        .toLowerCase()
        .replaceAll('-', '')
        .replaceAll(' ', '');
  ...
  Future<List<RuleCategory>> loadDataFromJson(String jsonPath) async {
    try {
      ...
      rootMap.forEach((keyName, content) {
        ...
        bool isGeneral =
            generalHeaders.contains(keyName) ||
            (keyName.toLowerCase().contains("traits") &&
                !keyName.contains("("));
        ...
        if (isGeneral || content is! Map) {
          ...
          if (content[traitsKey] != null) {
            ...
            if (content['description'] != null)
              traitsMap['Description'] = content['description'];
        ...
        final displayTitle = _headerTitleMap[keyName] ?? keyName;
      ...
      categories.sort(
        (a, b) => b.isGeneralInfo ? 1 : (a.isGeneralInfo ? -1 : 0),
      );
      return categories;
    } catch (e) {
      return [];
  ...
  void _processTrait(
    ...
    if (widget.title == "Equipment" &&
        value.containsKey('description') &&
        name.toLowerCase() != 'description') {
      final String overviewTitle = name.toLowerCase() == 'description'
          ? _overviewLabel
          : "$name ($_overviewLabel)";
      _addRuleItem(
        items,
  ...
  void _addRuleItem(
    ...
    String displayTitle = name.toLowerCase() == 'description'
        ? _overviewLabel
        : name;
    ...
    if (widget.title == "Equipment" &&
      ...
      if (columns.isNotEmpty) {
        ...
        for (int i = 0; i < rowCount; i++) {
          ...
          final itemName = i < colList.length
              ? colList[i].toString()
              : "Item ${i + 1}";
        ...
        if (columns.isNotEmpty) {
          ...
          for (int i = 0; i < maxRows; i++) {
            ...
            for (var col in columns) {
              if (table[col] is List) {
                final List colList =
                    table[col] as List; // as List olarak zorluyoruz
  ...
  Widget build(BuildContext context) {
    ...
    if (_lastLocale != currentLocale) {
      ...
      _overviewLabel = currentLocale == 'tr' ? 'Genel Bakış' : 'Overview';
      _headerTitleMap = {
        'Racial Traits': context.tr('rulebook.header.racialTraits'),
        'Class Features': context.tr('rulebook.header.classFeatures'),
        'Spellcasting Features': context.tr('rulebook.header.spellcastingFeatures'),
        'Equipment Rules': context.tr('rulebook.header.equipmentRules'),
        'Monster Rules': context.tr('rulebook.header.monsterRules'),
      };
      _futureRuleData = loadDataFromJson(_resolveJsonPath(context));
    }
      ...
      body: FutureBuilder<List<RuleCategory>>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.isEmpty)
            return Center(child: Text(context.tr('noDataFound')));
              ...
              if (generalItems.isNotEmpty)
                SliverToBoxAdapter(
                  child: Padding(
                    ...
                    child: _buildFeaturedGeneralInfo(
                      context,
                      generalItems.first,
                    ),
                  ),
  ...
  Widget _buildFeaturedGeneralInfo(
